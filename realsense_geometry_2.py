...
def vision_func(frame, adaptiveThres, eye_roi, direction, mode=0):
    '''
    ...
    ## double ellipse
    image_gray_eye = cv2.cvtColor(eye_roi, cv2.COLOR_BGR2GRAY)
    blur_eye = cv2.GaussianBlur(image_gray_eye,(3,3),0)

    ret,thresh1_eye = cv2.threshold(blur_eye,150,255,cv2.THRESH_BINARY)
    opening_eye = cv2.morphologyEx(thresh1_eye, cv2.MORPH_OPEN, kernel)
    closing_eye = cv2.morphologyEx(opening_eye, cv2.MORPH_CLOSE, kernel)
    cv2.imshow("simple t150", closing_eye)
    contours_eye, hierarchy_eye = cv2.findContours(closing_eye, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
    hull_eye = []
    for i in range(len(contours_eye)):
        hull_eye.append(cv2.convexHull(contours_eye[i], False))  # 50个点返回一次, w和h要输入直径
    for con_eye in hull_eye:
        approx = cv2.approxPolyDP(con_eye, 0.01 * cv2.arcLength(con_eye,True),True)
        area = cv2.contourArea(con_eye)
        if(len(approx) > 10 and area > 250): 
            cx,cy,w,h,theta = fit_rotated_ellipse_ransac(con_eye.reshape(-1,2))
            cv2.ellipse(eye_roi,(int(cx),int(cy)),(int(w),int(h)),theta*180.0/np.pi,0.0,360.0,(0,255,255),3)
    ...
    for con in hull:
        ...
        if(len(approx) > 10 and area > 250): 
            ...
            cv2.ellipse(frame,(int(cx),int(cy)),(int(w),int(h)),theta*180.0/np.pi,0.0,360.0,(0,0,255),3)

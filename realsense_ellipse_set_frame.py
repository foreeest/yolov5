...
import numpy as np
import math
import realsense_geometry_set_frame as geometry
...
from realsense_geometry_set_frame import Direction
...
def run(
    ...
    ## 获取ground truth
    frame_counter = 0
    target_frame = [1, 51, 101, 151, 201, 251, 301]
    target_frame_pointer = 1
    to_save_frame = np.zeros((1080, 1920, 3), dtype=np.uint8)
    ellipse_buffer = []
    vector_buffer = []
    temp_frame = np.zeros((1080, 1920, 3), dtype=np.uint8)
    ## end 获取ground truth
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image； det应该是预测结果  ;pred 可以在with里定义？  
            ...
            if len(det) == 2: # Assume that only 1 Eye and 1 Iris
                ...
                limbus, flag, temp_res = geometry.vision_func(roi, adaptiveThres, direction, MODE)
                if flag:
                    post = limbus[0]
                    dire = limbus[1] 
                    ...
                    ## 3d ground truth
                    to_save_frame = im_gv_ellipse_show.copy()
                    ## end 3d ground truth
                    ...
                    ## 取关键帧
                    print(f"key frame {target_frame_pointer} {frame_counter} {len(target_frame)}")
                    if target_frame_pointer < len(target_frame) and target_frame[target_frame_pointer] == frame_counter:
                        if target_frame_pointer != len(target_frame) - 1:
                            target_frame_pointer += 1
                        if target_frame_pointer == 4:
                            temp_frame = im0.copy()
                        ellipse_buffer.append(temp_res)
                        vec = np.array([int(temp_res[0]), int(temp_res[1]), end_point_vector_x, end_point_vector_y, eyeX, eyeY])
                        vector_buffer.append(vec)
                        cv2.imwrite(f"0907_1_{frame_counter}.jpg",to_save_frame)
                        text_file_path = '0907_1_ours.txt'
                        with open(text_file_path, 'a+') as text_file:
                            # text_content = f"{temp_res[0]} {temp_res[1]} {temp_res[2]} {temp_res[3]} {temp_res[4]} {post[0]} {post[1]} {post[2]} {dire[0]} {dire[1]} {dire[2]}\n"
                            text_content = f"{post[0]} {post[1]} {post[2]} {dire[0]} {dire[1]} {dire[2]}\n"
                            text_file.write(text_content)
                    if target_frame_pointer == len(target_frame) - 1 and target_frame[target_frame_pointer] == frame_counter:
                        deepvog_img = temp_frame.copy()
                        for index in range(0, len(ellipse_buffer)):
                            cv2.ellipse(deepvog_img,(int(ellipse_buffer[index][0]),int(ellipse_buffer[index][1])),(int(ellipse_buffer[index][2]),int(ellipse_buffer[index][3])),ellipse_buffer[index][4],0.0,360.0,(0,255,0),1)
                        for index in range(0, len(ellipse_buffer)):
                            # length_times = 2
                            # irisX, irisY, eyeX, eyeY = vector_buffer[index][0], vector_buffer[index][1], vector_buffer[index][4], vector_buffer[index][5]
                            # cv2.line(deepvog_img, (eyeX, eyeY), (irisX, irisY), color=(0, 255, 255), thickness=1)
                            # cv2.arrowedLine(deepvog_img, (irisX,irisY), (int((1+length_times)*irisX-length_times*eyeX), int((1+length_times)*irisY-length_times*eyeY)), color=(255, 0, 255), thickness=2, line_type=8, tipLength=0.2)
                            cv2.arrowedLine(deepvog_img, (vector_buffer[index][0],vector_buffer[index][1]), (vector_buffer[index][2],vector_buffer[index][3]), color=(255, 0, 255), thickness=3, line_type=8, tipLength=0.2)  
                        cv2.imwrite(f"0907_1_deepvog.jpg",deepvog_img)
                    ## end 取关键帧

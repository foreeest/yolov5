...
import numpy as np
import math
import realsense_geometry_2 as geometry
...
from realsense_geometry_2 import Direction
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image； det应该是预测结果  ;pred 可以在with里定义？  
            ...
            if len(det) == 2: # Assume that only 1 Eye and 1 Iris
                ...
                ## double ellipse
                m1_large = int(max(m1 - (m2 - m1) * 0.1, 0))
                m2_large = int(min(m2 + (m2 - m1) * 0.1, im0.shape[1]-1))
                n1_large = int(max(n1 - (n2 - n1) * 0.1, 0))
                n2_large = int(min(n2 + (n2 - n1) * 0.1, im0.shape[0]-1))
                eye_roi = im0[int(n1_large):int(n2_large), int(m1_large):int(m2_large)]
                # eye_roi = im0[int(n1):int(n2), int(m1):int(m2)]
                ## end double ellipse
                limbus, flag, temp_res = geometry.vision_func(roi, adaptiveThres, eye_roi, direction, MODE)
                if flag:
                    post = limbus[0]
                    dire = limbus[1] 
                    ...
                    cv2.line(im_gv_yolo_show, start_point_line, end_point_line, color=(0, 255, 0), thickness=2)
                    ...
                    length_scaler = 80
                    start_point_vector_ellipse = (irisX, irisY)
                    end_point_vector_x, end_point_vector_y = int(irisX + length_scaler * dire[0]), int(irisY + length_scaler * dire[1])
                    ...
                    if key == ord('s'): 
                        ...
                        directory_path = '.'
                        target_image_filename = "image_{:04d}.jpg"
                        target_image_path = os.path.join(directory_path, target_image_filename.format(file_counter))
                        cv2.imwrite(target_image_path, im_gv_yolo_show) 
                        print("Image saved")
                    # output -> to check
                    # global file_counter
                    # file_counter += 1
